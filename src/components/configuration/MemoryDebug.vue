  ...
  <div class="space-y-12">
    ...
    <div class="cfg-section">
      ...
      <div
        ...
        class="p-4 rounded-lg bg-primary/10 border border-primary/30"
      >
...
function severityBadgeClass(row: GrowthStat): string {
  ...
  if (s === 'low') return 'bg-accent-cyan/15 text-accent-cyan border border-accent-cyan/20';
...
function severityCardClass(row: GrowthStat): string {
  const s = getSeverity(row);
  if (s === 'critical') return 'border-accent-red/40 bg-accent-red/5';
  if (s === 'warning') return 'border-accent-amber/40 bg-accent-amber/5';
...
function severityTextClass(row: GrowthStat): string {
  ...
  if (s === 'low') return 'text-accent-cyan';
...
const overallHealthClass = computed(() => {
  ...
  if (s === 'critical') return 'border-accent-red/40 bg-accent-red/10 text-accent-red';
  if (s === 'warning') return 'border-accent-amber/40 bg-accent-amber/10 text-accent-amber';
  if (s === 'low') return 'border-accent-cyan/30 bg-accent-cyan/10 text-accent-cyan';
...
const overallHealthIcon = computed(() => {
  ...
  if (s === 'low')
    return '<svg class="w-5 h-5 text-accent-cyan" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>';
