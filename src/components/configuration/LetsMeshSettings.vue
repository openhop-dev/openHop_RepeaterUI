    ...
    <div class="cfg-section">
      ...
      <div v-else class="space-y-3">
        <div class="flex items-center gap-2">
          ...
          <span :class="['inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium', status.handler_active ? 'bg-accent-green/15 text-accent-green' : 'bg-background-mute text-content-muted']">
            <span class="w-1.5 h-1.5 rounded-full" :class="status.handler_active ? 'status-dot-green' : 'status-dot-muted'"></span>
        ...
        <div v-if="status.brokers.length" class="space-y-2">
          <div v-for="broker in status.brokers" :key="broker.host" class="flex items-center gap-2">
            ...
            <span :class="['inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium', broker.status.connected ? 'bg-accent-green/15 text-accent-green' : broker.status.reconnecting ? 'bg-accent-amber/15 text-accent-amber' : 'bg-accent-red/15 text-accent-red']">
              <span class="w-1.5 h-1.5 rounded-full" :class="broker.status.connected ? 'status-dot-green' : broker.status.reconnecting ? 'status-dot-amber' : 'status-dot-red'"></span>
        ...
        >
          ...
          <div class="flex items-center gap-3 px-4 py-2.5">
            <div class="min-w-0 flex-1 flex items-center gap-2 flex-wrap">
              <span :class="['inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium', broker.enabled ? 'bg-accent-green/15 text-accent-green' : 'bg-accent-red/15 text-accent-red']">
                <span class="w-1.5 h-1.5 rounded-full" :class="broker.enabled ? 'status-dot-green' : 'status-dot-red'"></span>
            ...
            <div v-if="isGlobalEditing" class="flex items-center gap-1.5 flex-shrink-0">
              ...
              <button
                ...
                title="Remove"
                class="p-1.5 rounded hover:bg-accent-red/10 text-content-secondary hover:text-accent-red transition-colors"
              >
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
