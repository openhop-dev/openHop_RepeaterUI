# Claude Code instructions for openHop-RepeaterUI
...
### Always ask before acting on non-trivial decisions

- **Never `git push` without explicit user approval for that specific push.**
- After committing, state what was committed and ask "Shall I push?" — do not push in the same step.
- A prior push approval does not carry over to subsequent commits. Ask each time.
- **Before making implementation decisions** — choosing a library vs hand-rolling, picking an icon approach, selecting a pattern — **propose the options and ask**. Do not silently pick one and implement it.
...
Misc:    code-block  (block monospace code/command display)
...
- **Always use the Lucide icon library (`@lucide/vue`) for icons.** Import named components (`import { Pin, ChevronDown } from '@lucide/vue'`) and render them as `<Pin class="w-4 h-4" />`.
- **Never draw icons as inline SVG blocks** unless the icon genuinely does not exist in Lucide. If you can't find an icon, check the [Lucide icon list](https://lucide.dev/icons/) before writing a custom SVG.
...
### Copy button
Always use `CopyLabel` + `useCopyToClipboard` — never write ad-hoc clipboard + setTimeout inline:
```vue
<script setup>
...
<template>
  <button @click="copy(someText)" class="btn-primary flex items-center gap-2">
    <svg>…</svg>
